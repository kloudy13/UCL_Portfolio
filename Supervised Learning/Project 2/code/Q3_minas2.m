...
for loop = 1:looplength;
    ...
    % generate the data
    [Yb,xb,kb] = GP(m,d,v0,e_amp,miou,sigma);
    ...
    for vloop = 1:length(av0);
        ...
        [alpha_i,yb_hat_valid] = km_krr(xbtrain,ybtrain,'gauss',av0(vloop),lambda,xbvalid);
        Kb = km_kernel(xbtrain,xbtrain,'gauss',av0(vloop));
        ...
        for c = 1:size(xbvalid,1);
            for d = 1:size(xbtrain,1);
                k_i(d) = exp(-(norm(xbvalid(c,:)-xbtrain(d,:),2)^2)/(2*av0(vloop)^2)); 
            end
        ...
        st_CDF = cumsum(hist(randn(size_valid,1),n_bins)/size_valid);
        % add distance to counter
        ecdf_dist(vloop) = sum(abs(eCDF_i-st_CDF));
    end
    % select best v
...
%     [apred,Ypred] = km_krr(xbtrain,ybtrain,'gauss',av0(v_pos(loop)),lambda,xbtest);
end
