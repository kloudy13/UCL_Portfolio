...
Part 8 task:
I will try using Sarsa instead of Q-learning. Compare the learning curves and final 
performance to your previous agent. (Plot the test performance and bellman loss).
"""
# ============================================================================
# ========================== IMPORTANT !!!! ==================================
# SETUP to save/load
...
batch_size = 300 # chosen after trial and error 
...
# for load and save:
folder = "models/"
file = "PartA8_ "+"LR:" + str(learn_rate)
...
def MakeBatch2(data):
#make batches of data coming in the form : [old_observation, observation, action, reward, done, act_next]
    ...
    for i in range(1,len(data)):
        ...
        batch_act_next = np.vstack((batch_act_next, data[i][5]))
    return batch_state_now, batch_state_next, batch_act, batch_r, batch_done, batch_act_next 
    
def MakeBuffer2(episodes):
# function to make random uniform data for the episodes   
    ...
    for i_episode in range(episodes):
        ...
        while c < (max_len):
            ...
            action1 = action # save old action to append to data
           #  update 
            obs, rew, done, _ = env.step(action)
            c += 1 # update counter
            if done:
                rew = -1 # set final reward
                data.Plus([obs1.reshape([1,4]), obs.reshape([1,4]), action1, rew, (not done)*1, action])
                break 
            else:
...
# class will hold the experineces (to make the experinec replay buffer)
    ...
    def RandBuff(self, batch_size):
        ...
        return np.reshape(np.array(random.sample(self.sth, batch_size)), [batch_size, 6])
...
# placeholder for predicted action
act_next = tf.placeholder(tf.float32, shape=[None, poss_action]) 
...
if Train == True:   
    with tf.Session() as sess: # start trianing
        ...
        for i in range(episodes):
                ...
                pred_act = sess.run(max_act, feed_dict = {state_now: obs.reshape(1,4)})
                ...
                while cc < (max_len):
                    ...
                    ReBuff.Plus([obs_temp, obs, act_temp, rew, (not done)*1, pred_act])
                    if done:
                        break
                    
                    # set new action from above learned results:
                    pred_act = sess.run(max_act, feed_dict =  {state_now: obs})
                    pred_act = MYgreed(epsilon, pred_act[0])
                        
   # trian network using batches
                    # make batches of experience:
                    batch_state, batch_next_state, batch_act, batch_r, batch_done, batch_pred_act = \
                    MakeBatch2(ReBuff.RandBuff(batch_size))
                    
                    # store two actions (old and predicted)
                    action = []
                    predact = []
                    for ii in range(batch_size):
                        action.append([ii, batch_act[ii]])
                    for jj in range(batch_size):
                        predact.append([jj, batch_act[jj]])
                    # define dictionary to feed to TF graph
                    feed = {state_now: batch_state, state_next: batch_next_state, act: action, r: batch_r, isdone: batch_done, w_tgt_1: w_copy1, w_tgt_h: w_copyh, act_next: predact}
  ...
  # EVALUATE                                             
                ...
                if i%20 == 0:  # evaluate every 20 episodes 
                    ...
                    for _ in range(eval_ep_N):
                        ...
                        while cc < (max_len):
                            ...
                            predact = [[0, pred_act]]
                            # define dictionary to feed to TF graph
                            feed = {state_now: obs_temp.reshape(1,4), state_next: obs.reshape(1,4), act: action, r: reward, isdone: done2, w_tgt_1: w_copy1, w_tgt_h: w_copyh, act_next: predact}
                            #  caluclate loss (loss)
    ...
    params =  "PartA8, Learning_Rate: " + str(learn_rate)
