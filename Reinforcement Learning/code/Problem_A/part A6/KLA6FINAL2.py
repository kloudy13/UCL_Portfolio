...
Start with the 100 unit network from question 4. Make an experience replay buffer 
for your agent that stores transitions of experience {(Si, Ai, Ri+1, Si+1)}ti=0. 
Train your agent with the replay buffer and compare performance to your original agent. 
...
# for load and save:
folder = "models/"
file = "tryPartA6_ "+"LR:" + str(learn_rate)
...
if Train == True:   
    with tf.Session() as sess: # start trianing
        ...
        for i in range(episodes):
                ...
                while cc < (max_len): # at most 300 moves
                    ...
                    pred_act = sess.run(max_act, feed_dict = feed)
                  # agent's action prediction:
                    ...
                    if done & (cc < max_len):
                     ...
                     # add generated state vector to the replay buffer:
   ...
   # trian network using batches
                    ...
                    feed = {state_now: batch_state, state_next: batch_next_state, act: action, r: batch_r, isdone: batch_done}
                    up = sess.run(update, feed_dict = feed)
      # EVALUATE                                          
                ...
                if i%20 == 0:  # evaluate every 20 episodes 
                    ...
                    for _ in range(eval_ep_N):
                        ...
                        while cc < (max_len):
                            ...
                            done2 = [[(not done)*1]]
                            # define dictionary to feed to TF graph
                            feed = {state_now: obs_temp.reshape(1,4), state_next: obs.reshape(1,4), act: action, r: reward, isdone: done2}
    ...
    params =  "PartA6, Learning_Rate: " + str(learn_rate)
