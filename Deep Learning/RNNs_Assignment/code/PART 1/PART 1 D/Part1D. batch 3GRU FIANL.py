...
# string1 is the title reference
    mtp.figure(1)    
    mtp.plot(inpt)
    mtp.title('MNIST - Part 1D 3xGRU '+string1)    
...
def recurrent_neural_network3(x, y): 
    ...
    # not implemneting dropout as no improvemnt in results    
...
n_epochs = 20
batch_size = 500
learning_rate = 0.005 # better than AdamOptimiser defaulttrain = True
train = True
n_layer = 3 # number of cells

# for loading/ saving:
fileName = "BN_Model_Part1D_" + str(rnn_size) +'.'+ str(batch_size) +'.'+ str(learning_rate)
...
clipped_temp = [(tf.clip_by_value(grad, -0.9, 0.9), var) for grad, var in temp] 
    # clipping params (min/max value) chosen after some trial and error  
