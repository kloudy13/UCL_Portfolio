...
# string1 is the title reference
    mtp.figure(1)    
    mtp.plot(inpt)
    mtp.title('MNIST - Part 1C 3xLSTM '+string1)    
...
def recurrent_neural_network3(x,y): 
    ...
    cell = tf.nn.rnn_cell.MultiRNNCell([cell]* n_layer, state_is_tuple=True) #stack  by num layers
...
rnn_size = 32 # change to 64/128 to run model with different no. units
n_epochs = 20
batch_size = 500 #bad at 250.0.02
learning_rate = 0.0005 
n_layer = 3 # number of cells
drop_prob = 1 # dropout rate, set to 1 as does not improve results 

# for loading/ saving:
fileName = "BN_Model_Part1C_" + str(rnn_size) +'.'+ str(batch_size) +'.'+ str(learning_rate)
...
optimum = tf.train.AdamOptimizer(learning_rate = learning_rate, beta1=0.95, beta2=0.99, epsilon=1e-09, use_locking=False)  
...
## ---------------------------- LOAD MODEL -----------------------------------   
    ...
    print('Commence loading:'+ fileName)
