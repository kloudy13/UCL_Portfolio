...
# CONTROL 
# chose units 
rnn_size = 128 # change to 32/64/128 to run model with different no. units
...
# string1 is the title reference
    mtp.figure(1)    
    mtp.plot(inpt)
    mtp.title('MNIST - Part 1B GRU '+string1)    
...
#------------------------------------------------------------------------------   
#   BUILD GRAPH:
def recurrent_neural_network3(x,y,Train): 
    # a and y are placeholders
...
# for loading/ saving:
fileName = "BN_Model_Part1B_" + str(rnn_size) +'.'+ str(batch_size) +'.'+ str(learning_rate)
...
#  MODEL SETUP    
y_prediction = recurrent_neural_network3(x,y,train)
