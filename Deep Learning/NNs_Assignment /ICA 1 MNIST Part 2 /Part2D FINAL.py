...
class CrossEntropy(object):
    ...
    # modify by adding a small constant to the  activation (a) 
    # prevents numerical underflow
        out = np.mean(np.sum(-y * np.log(activ+(1e-6)), axis=1))
        return (out)        
...
# ----------------------------------------------------------------------------
# make confussion matrix - needs list of np.arrays as input (labels var)
...
# ========================= PART == 2D === SOLUTION ============================
# 2 hidden layers (256 units) each, with ReLU non-linearity, follow by a softmax
...
# ============================ Load dataset ===================================
    ...
    dim_batch = 50  # batch size
    max_iter = 50 # epochs
    # initialise lists to be stored
    trainAcc = []
    testAcc = []

 ### Define the neural network Part 2D:
    # Input layer  
    dimH1, dimH2 = 256, 256
    LinLayer1 =  LinLayer(dim_x, dimH1, sd = 0.01) 
    HiddLayer1 = ReLU()
    
    LinLayer2 = LinLayer(dimH1, dimH2, sd = 0.01)  
    HiddLayer2 = ReLU()
    
    LinLayer3 = LinLayer(dimH2, dim_y, sd = 0.01)  
    ...
    for i in range(max_iter):
        ...
        for batch in range(Train_dat_inst//dim_batch):
           ...
           # pass forward B
            ...
            activat =  HiddLayer1.ForPass(lin1_out)
            lin2_out = LinLayer2.ForPass(activat)
            activat2 =  HiddLayer2.ForPass(lin2_out)
            lin3_out = LinLayer3.ForPass(activat2)
            batch_train_y_pred = SoftMax_out.ForPass(lin3_out)
          
          # backward pass            
            bck = Loss.BackPass(batch_train_y_pred, batch_train_y)
            bck1 = SoftMax_out.BackPass(bck)
            bck2 = LinLayer3.BackPass(bck1)
            bck3 = HiddLayer2.BackPass(bck2)
            bck4= LinLayer2.BackPass(bck3)
            bck5 = HiddLayer1.BackPass(bck4)
            bck6 = LinLayer1.BackPass(bck5)
        # perform gradient descent
            LinLayer1.Update(rate)
            LinLayer2.Update(rate)
            LinLayer3.Update(rate)
            
       # pass forward
        lin1_out =  LinLayer1.ForPass(train_x)
        activat1 =  HiddLayer1.ForPass(lin1_out)
        lin2_out = LinLayer2.ForPass(activat1)
        activat2 =  HiddLayer2.ForPass(lin2_out)
        lin3_out = LinLayer3.ForPass(activat2)
        train_y_pred = SoftMax_out.ForPass(lin3_out)
        ...
        activat2 =  HiddLayer2.ForPass(lin2_out)
        lin3_out = LinLayer3.ForPass(activat2)
        test_y_pred = SoftMax_out.ForPass(lin3_out)
        w ,b = LinLayer3.WeightsOut() # get final weights
...
### ------------------------------ RESULTS ------------------------------------
# Plot
make_plot2(trainAcc, "D", "  Train")
make_plot2(testAcc, "D", "  Test")
